"""
Train CBF controller for 5-agent quadrotor swarm using CVXPyLayers.

Matches reference train.py --problem quadcopter_multi exactly:
  - 5 agents, 12-state quadrotor per agent (state ∈ R^60)
  - 3 spherical obstacles
  - Initial: x∈[1.1,1.9], y=-0.5, z=1.0, noise std=4e-2 on xy
  - Target: x∈[1.1,1.9], y=3.5, z=1.0
  - Running cost: dt*(0.5*||u||² + ||v||²) summed over T, mean over batch
  - Terminal cost: 0.5*(||pos-target||² + ||vel||² + ||angles||² + ||ang_vel||²)
  - Alpha annealing: starts 20, +5 every 20 epochs, capped at 200
  - T=10.0, dt=0.2, cbf_alpha=(1,1), lr=1e-3, weight_decay=1e-3
"""
...
# ============================================================================
# Hooks (identical structure to train_quadrotor_multi_cvxpy.py)
# ============================================================================
...
def make_output_transform(n_agent, T_hover, T_dev_scale, tau_scale, device, dtype):
    ...
    def output_transform(u_raw):
        ...
        u_scaled = torch.cat([
            T_dev_scale * torch.tanh(u[:, :, 0:1]),
            tau_scale   * torch.tanh(u[:, :, 1:4]),
...
def make_cost_fn(p_target, alpha_running, n_agent, dt=0.2):
    """
    Matches reference quadcopter_multi.py compute_loss (n_agent=5):
    ...
    def cost_fn(states, controls, alpha_terminal):
        ...
        vel_penalty = (
            ...
            .pow(2).sum(dim=-1).sum(dim=-1)
        )
        run_control = dt * 0.5 * (controls ** 2).sum(dim=-1).sum(dim=-1)
        run_vel     = dt * vel_penalty.sum(dim=-1)
...
def make_plot_fn(dynamics, obstacles, p_target, plot_dir):
    ...
    def plot_fn(_trainer, epoch, states):
        ...
        dynamics.plot_trajectory(
            ...
            p_target=p_target,
            save_path=str(path),
            title=f"Epoch {epoch} - Quadrotor Trajectories (5-agent)"
        )
...
def train_quadrotor_multi(args):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    dtype = torch.float64
    torch.manual_seed(args.seed)

    # --- System parameters (matching reference quadcopter_multi.py) ---
    n_agent = 5
    mass    = 0.5
    gravity = 1.0
    ...
    # --- Obstacles (same 3 as swarm, matching train.py:433-438) ---
    obstacle_configs = [
    ...
    # --- Target positions (n_agent=5: x∈[1.1,1.9], y=3.5, z=1.0) ---
    ...
    p_target[:, 1] = 3.5
    p_target[:, 2] = 1.0   # quadcopter_multi: z_target=1.0

    # --- Initial state (x∈[1.1,1.9], y=-0.5, z=1.0) ---
    ...
    config = TrainingConfig(
        ...
        # CBF — (1,1) matches reference gamma(x)=x
        cbf_alpha=(1.0, 1.0),
        use_slack=args.use_slack,
        slack_penalty=args.slack_penalty,
        velocity_penalty=False,

        # Cost — quadcopter_multi annealing: cap at 200 (train.py:447)
        control_penalty=1.0,
        terminal_cost_weight=20.0,
        alpha_terminal_final=200.0,       # cap at 200 (train.py:447: alpha_terminal_final=2e2)
        alpha_terminal_step=5.0,
        alpha_terminal_every=20,          # +5 every 20 epochs (train.py:568)
    ...
    print("\n" + "=" * 70)
    print("TRAINING: 5-Agent Quadrotor (CVXPyLayers + HOCBF)")
    print("=" * 70)
    ...
    # --- Save final model ---
    torch.save(trainer.policy.state_dict(), save_dir / "final_model.pth")
    print(f"Final model: {save_dir / 'final_model.pth'}")
    print(f"Best model:  {save_dir / 'best_model.pth'}  (loss={trainer.best_loss:.4e})")

    # --- Final trajectory plot ---
    with torch.no_grad():
        z0 = initial_state.unsqueeze(0)
        final_states, _ = trainer.rollout(z0)
    traj_final = final_states.permute(0, 2, 1)
    dynamics.plot_trajectory(
        traj=traj_final,
        obstacles=obstacles,
        p_target=p_target,
        save_path=str(save_dir / "final_trajectory.png"),
        title="Final Trajectory - 5-Agent Quadrotor"
    )
